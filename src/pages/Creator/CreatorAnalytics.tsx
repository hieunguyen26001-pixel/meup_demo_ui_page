...
type SortField = 'creatorName' | 'totalOrders' | 'totalRevenue' | 'cancelRate' | 'video.orders' | 'video.revenue' | 'video.cancelRate' | 'display.orders' | 'display.revenue' | 'display.cancelRate' | 'external.orders' | 'external.cancelRate';
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                  Thống kê Nguồn doanh thu của creator
                </h3>
              </div>
            ...
            <table className="w-full min-w-[1000px] border-collapse">
              <thead>
                ...
                <tr className="bg-gray-50 dark:bg-gray-800/50">
                  ...
                  <th colSpan={3} className="px-4 py-3 text-center text-sm font-semibold text-white border border-blue-300 dark:border-blue-600 bg-gradient-to-r from-blue-500 to-blue-600">
                    Video
                  </th>
                  <th colSpan={3} className="px-4 py-3 text-center text-sm font-semibold text-white border border-green-300 dark:border-green-600 bg-gradient-to-r from-green-500 to-green-600">
                    Trưng bày
                  </th>
                  <th colSpan={2} className="px-4 py-3 text-center text-sm font-semibold text-white border border-purple-300 dark:border-purple-600 bg-gradient-to-r from-purple-500 to-purple-600">
                    Phát trực tiếp
                  </th>
                ...
                <tr className="bg-gray-100/50 dark:bg-gray-800/20">
                  ...
                  <th 
                    className="px-2 py-2 text-center text-xs font-medium text-purple-800 border border-purple-200 w-[80px] bg-purple-50 dark:bg-purple-900/20 cursor-pointer hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors select-none"
                    onClick={() => handleSort('external.cancelRate')}
                  >
                    <div className="flex items-center justify-center gap-1">
                      Tỷ lệ hủy
                      {sortField === 'external.cancelRate' && (
                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                          {sortDirection === 'asc' ? (
                            <path fillRule="evenodd" d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clipRule="evenodd" />
                          ) : (
              ...
              <tbody>
                {currentData.map((creator, index) => (
                  <tr key={creator.creatorName} className={`${index % 2 === 0 ? 'bg-gray-50/30 dark:bg-gray-800/10' : 'bg-white dark:bg-gray-800'} hover:bg-gray-100/50 dark:hover:bg-gray-800/20 transition-colors`}>
                    ...
                    <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 w-[80px]">
                      {creator.external.orders > 0 ? creator.external.cancelRate.toFixed(1) + '%' : '0%'}
                    </td>
                ...
                <tr className="bg-gray-100 dark:bg-gray-800/50 font-semibold">
                  ...
                  <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20">
                    {(() => {
                      const totalVideoOrders = creatorStats.reduce((sum, creator) => sum + creator.video.orders, 0);
                      const totalVideoCancelled = creatorStats.reduce((sum, creator) => {
                        const creatorOrders = processedData.filter(order => 
                          order.creatorUsername === creator.creatorName && 
                          order.contentType === 'Video'
                        );
                        return sum + creatorOrders.filter(order => order.orderStatus === 'cancelled').length;
                      }, 0);
                      return totalVideoOrders > 0 ? ((totalVideoCancelled / totalVideoOrders) * 100).toFixed(1) + '%' : '0%';
                    })()}
                  </td>
                  <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20">
                  ...
                  <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20">
                    {(() => {
                      const totalDisplayOrders = creatorStats.reduce((sum, creator) => sum + creator.display.orders, 0);
                      const totalDisplayCancelled = creatorStats.reduce((sum, creator) => {
                        const creatorOrders = processedData.filter(order => 
                          order.creatorUsername === creator.creatorName && 
                          order.contentType === 'Display'
                        );
                        return sum + creatorOrders.filter(order => order.orderStatus === 'cancelled').length;
                      }, 0);
                      return totalDisplayOrders > 0 ? ((totalDisplayCancelled / totalDisplayOrders) * 100).toFixed(1) + '%' : '0%';
                    })()}
                  </td>
                  <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20">
                  ...
                  <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20">
                    {(() => {
                      const totalExternalOrders = creatorStats.reduce((sum, creator) => sum + creator.external.orders, 0);
                      const totalExternalCancelled = creatorStats.reduce((sum, creator) => {
                        const creatorOrders = processedData.filter(order => 
                          order.creatorUsername === creator.creatorName && 
                          order.contentType === 'External'
                        );
                        return sum + creatorOrders.filter(order => order.orderStatus === 'cancelled').length;
                      }, 0);
                      return totalExternalOrders > 0 ? ((totalExternalCancelled / totalExternalOrders) * 100).toFixed(1) + '%' : '0%';
                    })()}
                  </td>
                  ...
                  <td className="px-2 py-2 text-sm text-gray-800 dark:text-gray-200 text-center border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/20">
                    {(() => {
                      const totalOrders = creatorStats.reduce((sum, creator) => sum + creator.totalOrders, 0);
                      const totalCancelled = processedData.filter(order => order.orderStatus === 'cancelled').length;
                      return totalOrders > 0 ? ((totalCancelled / totalOrders) * 100).toFixed(1) + '%' : '0%';
                    })()}
                  </td>
                  <td className="px-2 py-2 text-sm font-semibold text-blue-700 dark:text-blue-300 text-center border border-gray-200 dark:border-gray-700 bg-blue-100 dark:bg-blue-800/50">
                    {formatCurrency(creatorStats.reduce((sum, creator) => sum + creator.totalRevenue, 0))}
            ...
            <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-700 dark:text-gray-300">
                  Hiển thị {((validCurrentPage - 1) * itemsPerPage) + 1} đến {Math.min(validCurrentPage * itemsPerPage, sortedData.length)} trong tổng số {sortedData.length} creators
                </div>
                {totalPages > 1 && (
                  <div className="flex items-center gap-2">
                    ...
                    {(() => {
                      ...
                      let endPage = Math.min(totalPages, validCurrentPage + halfVisible);
                      ...
                      if (startPage > 1) {
                        pages.push(
                          <button
                            ...
                            className="px-3 py-1 text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors"
                          >
                            1
                          </button>
                        );
